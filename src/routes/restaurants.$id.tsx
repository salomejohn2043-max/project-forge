...
function GroupOrderCTA({ restaurantId, open }: { restaurantId: string; restaurantName: string; open: boolean }) {
  const { user } = useAuth();
  ...
  return (
    <div className="mt-4 flex items-center justify-between gap-3 rounded-xl border border-primary/30 bg-primary/5 p-3">
      ...
      <Button size="sm" disabled={busy} onClick={async () => {
        ...
        const address = window.prompt("Delivery address for the group?", "");
